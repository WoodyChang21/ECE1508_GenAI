...
2. **We already have a substitute for risk control, at least for PatchTST: we only trade long when its
   ...
   *reactive* risk control (react once price moves against you); a quality gate is a *selective* one
   ...
   honestly (point 1), leaning on the selective kind — rather than forcing in a mechanism we can't fairly
   evaluate — was the more defensible choice for this backtest. This argument is weaker for CVAE now that
   it has no quality gate at all (see above) — its only selectivity is the return-size threshold, which
   is a different kind of filter (screens on predicted edge *size*, not on model uncertainty).
...
candle's close regardless, if it doesn't.** No stop-loss. See `backlog.md`
...
impossible in principle. A later, differently-motivated pass (fixed rather than mirrored, sized off the
instrument's own empirical volatility bound rather than off the take-profit) is discussed briefly in
*Next steps* and logged in `backlog.md`; it was explored ad hoc, not implemented in the shipped backtest,
and would need re-checking against the current no-gate CVAE baseline before drawing conclusions from it.
...
correcting once when a retrain shifted its predicted-edge scale (see `backlog.md`).
...
### Win — take-profit hit (`ctx_bars=70`, `start_idx=26768`)

![win_take_profit](outputs/sample_plots/win_take_profit_start26768_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 551.38 / 550.96 | 550.97 / 547.28 | 547.22 / 546.02 | 550.55 | — | — | **BENCHMARK** → sell 546.02 (**−0.82%**) |
| PatchTST | 550.76 / 550.45 | 552.50 / 551.83 | 551.14 / 550.06 | 550.55 | 551.83 | **NO TRADE** | — |
| CVAE | 550.41 / 550.57 | 550.90 / 551.07 | 550.52 / 550.86 | 550.55 | 550.68 | **ENTER** | **TAKE PROFIT** → sell 550.68 (**+0.02%**) |

### Lose — expiry (loss) (`ctx_bars=70`, `start_idx=24796`)

![lose_expiry](outputs/sample_plots/lose_expiry_start24796_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 510.48 / 509.00 | 508.99 / 509.49 | 509.48 / 510.55 | 511.76 | — | — | **BENCHMARK** → sell 510.55 (**−0.24%**) |
| PatchTST | 511.61 / 512.05 | 512.55 / 511.95 | 512.36 / 512.38 | 511.76 | 512.38 | **ENTER** | **EXPIRED** → sell 510.55 (**−0.24%**) |
| CVAE | 511.70 / 511.88 | 512.06 / 512.17 | 511.91 / 512.10 | 511.76 | 511.88 | **ENTER** | **EXPIRED** → sell 510.55 (**−0.24%**) |

### Skipped (quality gate or return too small) (`ctx_bars=70`, `start_idx=25605`)

![skipped](outputs/sample_plots/skipped_start25605_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 563.18 / 561.90 | 561.90 / 560.64 | 560.63 / 561.15 | 562.09 | — | — | **BENCHMARK** → sell 561.15 (**−0.17%**) |
| PatchTST | 561.91 / 562.27 | 563.04 / 562.94 | 562.39 / 561.83 | 562.09 | 562.94 | **NO TRADE** | — |
| CVAE | 561.98 / 562.06 | 562.30 / 562.39 | 562.06 / 562.28 | 562.09 | 562.10 | **NO TRADE** | — |

### No trade (target <= buy) (`ctx_bars=70`, `start_idx=25125`)

![no_trade](outputs/sample_plots/no_trade_start25125_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 529.88 / 530.63 | 530.64 / 531.14 | 531.14 / 530.51 | 529.76 | — | — | **BENCHMARK** → sell 530.51 (**+0.14%**) |
| PatchTST | 528.39 / 528.50 | 530.39 / 529.58 | 530.65 / 530.65 | 529.76 | 530.65 | **NO TRADE** | — |
| CVAE | 529.65 / 529.68 | 529.84 / 529.82 | 529.49 / 529.65 | 529.76 | 529.68 | **NO TRADE** | — |
...
|---|---|---|---|
| Win — take-profit hit | 5.36 | 2.44 | 0.65 |
| Lose — expiry (loss) | 1.56 | 0.94 | 0.47 |
| Skipped (quality gate or return too small) | 2.55 | 1.21 | 0.41 |
| No trade (target <= buy) | 1.26 | 2.26 | 0.34 |
...
| PatchTST walk-forward | 460 | 63.5% | 41.5% | +0.016% | +7.14% | +5.17% |
| CVAE walk-forward | 684 | 88.3% | 88.3% | +0.007% | +3.95% | +2.87% |
...
- [ ] Revisit a fixed, volatility-calibrated stop-loss (sized off the instrument's own empirical bound
  rather than mirrored to the take-profit target) against the current no-gate CVAE baseline — an earlier
  ad hoc pass against the prior (gated) baseline found a 2% stop-loss reduced CVAE's loss without the
  return-asymmetry side effects that broke the mirrored 1:1 version, but this was never implemented in the
  shipped backtest or re-checked after the gate was removed (see `backlog.md`).
