...
- **Quality gate**: each model has its own, independently-defined "is this worth trading" check — not a
  shared score. CVAE's is a genuine 0–1 confidence dial (see below); PatchTST's is a plain yes/no with
  nothing to tune. An earlier version squeezed both onto one shared 0–1 "confidence" scale and applied one
  threshold to both, which implied a comparability neither model actually had — see *Trading criteria*
...
2. **Only enter if the take-profit clears the buy price by enough to bother.** Two separate gates:
   ...
   - The predicted edge (`take_profit / buy_price - 1`) must also clear a **minimum return threshold** —
     ...
     **per model**, not shared: `--patchtst-min-return-threshold` (default 0.1%) and
     ...
     whose quality gate (below) doesn't pass.
3. **The model's own quality gate must also pass** — this is *not* a shared score across models (see
   *Key terms*): PatchTST's is a plain boolean, CVAE's is its own thresholded confidence value
   (`--cvae-consensus-threshold`, default 0.5) — see below for exactly how each is computed. A window can
   fail this gate even after clearing both of the above.
...
Each model's quality gate decides *which* windows are worth trading in the first place, on top of the
return-threshold gate above — deliberately **not** a shared score, after an earlier version tried to
squeeze both models onto one 0–1 "confidence" scale and apply a single shared threshold to it. That
implied the two numbers meant the same thing, which they never did; see `backlog.md`'s "trade confidence"
entry for the full reasoning behind dropping it. Instead:
- **CVAE**: what fraction of the *k* generated samples predict an upward move. Close to 1.0 = strong
  ...
  "don't bother trading this" cases at once, thresholded against its own `--cvae-consensus-threshold`
  (default 0.5). This is inherently a per-window quantity, computed fresh for every decision with no
  dependence on any other window.
- **PatchTST** (which only ever produces one guess, so there's no "agreement between samples" to
  measure): a plain boolean, not a score — do all 3 predicted candles agree on the up direction? Nothing
  to threshold or tune. An earlier version instead ranked the predicted move's magnitude against an
  online, expanding history of the walk's own prior decisions, to manufacture a 0–1 number comparable to
  CVAE's — dropped for being needless complexity in service of a comparability that didn't actually hold
  (and it had its own wrinkle: noisier confidence early in the walk, before the reference had accumulated
  much history). The plain boolean is simpler and doesn't pretend to be something it isn't.

Both the quality gate and the minimum-return threshold are fixed at a single value per model for this
...
2. **We already have a substitute for risk control: we only trade long when the model's own quality gate
   passes.** The whole point of that gate (see above) is to filter out exactly the windows where the
   model itself isn't sure — including cases that look like they might chop or reverse. A stop-loss is a
   *reactive* risk control (react once price moves against you); the quality gate is a *selective* one
   (don't enter in the first place if the setup looks shaky). Given we can't simulate the reactive kind
   honestly (point 1), leaning on the selective kind we already have — rather than forcing in a mechanism
   we can't fairly evaluate — is the more defensible choice for this backtest.
...
So the strategy as it stands: **enter long only when the model's own quality gate passes, the predicted
edge clears its own return threshold, and the model's own take-profit implies real upside; exit at the
take-profit if the real market reaches it, or at the 3rd real candle's close regardless, if it doesn't.**
No stop-loss. See `backlog.md`
...
### Win — take-profit hit (`ctx_bars=70`, `start_idx=25171`)

![win_take_profit](outputs/sample_plots/win_take_profit_start25171_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 529.61 / 529.43 | 529.43 / 528.93 | 528.93 / 529.50 | 529.63 | — | — | **BENCHMARK** → sell 529.50 (**−0.02%**) |
| PatchTST | 528.83 / 529.52 | 531.08 / 530.91 | 531.04 / 531.73 | 529.63 | 531.73 | **NO TRADE** | — |
| CVAE | 529.65 / 529.83 | 530.02 / 530.11 | 530.04 / 530.24 | 529.63 | 529.98 | **ENTER** | **TAKE PROFIT** → sell 529.98 (**+0.07%**) |
...
| CVAE | 582.72 / 582.82 | 583.09 / 583.19 | 582.94 / 583.19 | 582.84 | 582.97 | **ENTER** | **EXPIRED** → sell 582.85 (**+0.00%**) |

### Lose — expiry (loss) (`ctx_bars=70`, `start_idx=26627`)

![lose_expiry](outputs/sample_plots/lose_expiry_start26627_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 569.76 / 567.03 | 565.52 / 559.54 | 559.57 / 558.68 | 569.76 | — | — | **BENCHMARK** → sell 558.68 (**−1.94%**) |
| PatchTST | 568.29 / 569.63 | 570.39 / 570.20 | 570.35 / 569.09 | 569.76 | 570.20 | **NO TRADE** | — |
| CVAE | 569.64 / 569.79 | 570.15 / 570.32 | 569.94 / 570.27 | 569.76 | 570.01 | **ENTER** | **EXPIRED** → sell 558.68 (**−1.94%**) |

### Skipped (quality gate or return too small) (`ctx_bars=70`, `start_idx=26375`)

![skipped](outputs/sample_plots/skipped_start26375_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 601.09 / 601.76 | 600.64 / 600.63 | 600.62 / 601.27 | 601.09 | — | — | **BENCHMARK** → sell 601.27 (**+0.03%**) |
| PatchTST | 599.91 / 600.06 | 600.98 / 601.93 | 600.74 / 599.61 | 601.09 | 601.93 | **NO TRADE** | — |
| CVAE | 600.96 / 600.99 | 601.25 / 601.30 | 601.09 / 601.30 | 601.09 | 601.13 | **NO TRADE** | — |

### No trade (target <= buy) (`ctx_bars=70`, `start_idx=24998`)

![no_trade](outputs/sample_plots/no_trade_start24998_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 495.08 / 495.10 | 497.83 / 496.43 | 496.44 / 496.43 | 495.08 | — | — | **BENCHMARK** → sell 496.43 (**+0.27%**) |
| PatchTST | 494.18 / 494.35 | 494.27 / 495.02 | 495.56 / 495.44 | 495.08 | 495.44 | **NO TRADE** | — |
| CVAE | 494.94 / 495.07 | 495.15 / 495.14 | 495.02 / 495.10 | 495.08 | 495.06 | **NO TRADE** | — |
...
|---|---|---|---|
| Win — take-profit hit | 0.68 | 2.90 | 0.59 |
| Win — expiry (gain) | 1.56 | 2.24 | 0.47 |
| Lose — expiry (loss) | 11.08 | 2.10 | 0.68 |
| Skipped (quality gate or return too small) | 1.14 | 2.33 | 0.34 |
| No trade (target <= buy) | 2.75 | 1.38 | 0.21 |
...
5. **Each model's quality gate and minimum-return threshold are both fixed per model, not swept** (see
   *Trading criteria* above for why: any of them changes which decisions actually get made along the way,
   so a different value produces a genuinely different simulated path rather than a different slice of
   the same one).
...
| CVAE walk-forward | 663 | 86.7% | 86.4% | −0.002% | −1.67% | −1.22% |
...
| Case | PatchTST | CVAE |
|---|---|---|
| Win — take-profit hit | 12.9% | 53.5% |
| Win — expiry (gain) | 6.8% | 0.2% |
| Lose — expiry (loss) | 11.4% | 8.2% |
| Skipped (quality gate or return too small) | 64.3% | 25.8% |
| No trade (target <= buy) | 4.5% | 12.3% |
...
- **The two models' quality gates aren't comparable to each other, by design.** CVAE's is a genuine 0–1
  confidence dial ("how much do my samples agree with each other"); PatchTST's is a plain boolean ("do my
  3 predicted candles agree on direction"), with nothing to tune. An earlier version squeezed both onto
  one shared 0–1 scale with one shared threshold, which implied a comparability that didn't actually
  exist — dropped for exactly that reason (see `backlog.md`'s "trade confidence" entry). The tradeoff this
  introduces: CVAE has a continuous, tunable dial; PatchTST doesn't, so there's no way to ask PatchTST for
  "stricter coherence," and a single "how selective is each model's own gate" headline number is now
  structurally awkward to compare across the two.
- **The "skipped" case bundles two distinct reasons together.** A decision lands there whether it failed
  the model's own quality gate, the minimum-return gate, or both — the outcome-breakdown table can't
  currently tell you which. Splitting it into two separate cases is a natural follow-up (see *Next
  steps*).
...
**Weaknesses**
- Neither model's quality gate is guaranteed to behave like a calibrated "higher = better" dial by
  construction. CVAE's consensus fraction has at least shown real spread on the current checkpoint (0.2 to
  1.0 across percentiles — see the "training diagnostic" note under Loss), which is a good sign but not
  the same as confirming it's *calibrated*; that would need re-running the walk-forward backtest at
  several `--cvae-consensus-threshold` values and checking win rate actually improves as the threshold
  rises. PatchTST's gate is a fixed boolean now, so there's no threshold to sweep on that side at all.
...
- [ ] Sweep `--cvae-consensus-threshold`, `--patchtst-min-return-threshold`, and
  `--cvae-min-return-threshold` (all currently fixed) as full separate walk-forward runs, and/or let a
  trade re-enter as soon as an early take-profit fills instead of always
  locking out the full 3 bars — see *Limitations* above.
- [ ] Split the **skipped** case into two (failed quality gate vs. return too small) so the
  outcome-breakdown table can say which gate is actually doing the filtering.
