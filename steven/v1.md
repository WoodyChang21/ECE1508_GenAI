...
### Win — take-profit hit — narrow context (`ctx_bars=28`, `start_idx=25808`)

![win_take_profit_narrow](outputs/sample_plots/win_take_profit_narrow_start25808_ctx28.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 573.39 / 573.85 | 573.86 / 573.28 | 573.30 / 572.23 | 572.29 | — | — | **BENCHMARK** → sell 572.23 (**−0.01%**) |
| PatchTST | 582.76 / 593.75 | 562.77 / 573.56 | 561.50 / 572.26 | 572.29 | 593.75 | **ENTER** | **EXPIRED** → sell 572.23 (**−0.01%**) |
| CVAE | 574.69 / 574.06 | 574.84 / 570.84 | 568.18 / 563.71 | 572.29 | 572.59 | **ENTER** | **TAKE PROFIT** → sell 572.59 (**+0.05%**) |

### Win — take-profit hit — wide context (`ctx_bars=63`, `start_idx=25663`)

![win_take_profit_wide](outputs/sample_plots/win_take_profit_wide_start25663_ctx63.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 547.92 / 548.56 | 548.55 / 547.82 | 547.73 / 550.00 | 547.92 | — | — | **BENCHMARK** → sell 550.00 (**+0.38%**) |
| PatchTST | 558.18 / 568.79 | 538.37 / 544.83 | 537.62 / 540.27 | 547.92 | 568.79 | **ENTER** | **EXPIRED** → sell 550.00 (**+0.38%**) |
| CVAE | 544.36 / 538.02 | 545.09 / 545.56 | 547.89 / 547.74 | 547.92 | 550.11 | **ENTER** | **TAKE PROFIT** → sell 550.11 (**+0.40%**) |

### Win — expiry (gain) — narrow context (`ctx_bars=14`, `start_idx=26372`)

![win_expiry_narrow](outputs/sample_plots/win_expiry_narrow_start26372_ctx14.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 607.17 / 607.99 | 608.00 / 607.66 | 607.67 / 608.10 | 607.17 | — | — | **BENCHMARK** → sell 608.10 (**+0.15%**) |
| PatchTST | 618.26 / 630.10 | 600.87 / 611.33 | 596.44 / 607.60 | 607.17 | 630.10 | **ENTER** | **EXPIRED** → sell 608.10 (**+0.15%**) |
| CVAE | 606.37 / 602.95 | 599.07 / 597.60 | 606.72 / 611.72 | 607.17 | 609.97 | **ENTER** | **EXPIRED** → sell 608.10 (**+0.15%**) |

### Win — expiry (gain) — wide context (`ctx_bars=63`, `start_idx=26256`)

![win_expiry_wide](outputs/sample_plots/win_expiry_wide_start26256_ctx63.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 587.82 / 588.72 | 588.70 / 586.48 | 586.47 / 588.11 | 587.83 | — | — | **BENCHMARK** → sell 588.11 (**+0.05%**) |
| PatchTST | 599.13 / 602.39 | 593.87 / 600.93 | 576.74 / 587.83 | 587.83 | 602.39 | **ENTER** | **EXPIRED** → sell 588.11 (**+0.05%**) |
| CVAE | 593.73 / 588.63 | 585.79 / 592.46 | 588.18 / 594.56 | 587.83 | 590.73 | **ENTER** | **EXPIRED** → sell 588.11 (**+0.05%**) |

### No trade (target <= buy) — narrow context (`ctx_bars=14`, `start_idx=24920`)

![no_trade_narrow](outputs/sample_plots/no_trade_narrow_start24920_ctx14.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 522.51 / 521.69 | 521.70 / 521.27 | 519.80 / 520.44 | 522.51 | — | — | **BENCHMARK** → sell 520.44 (**−0.40%**) |
| PatchTST | 529.54 / 531.50 | 530.87 / 540.88 | 513.19 / 523.05 | 522.51 | 540.88 | **ENTER** | **EXPIRED** → sell 520.44 (**−0.40%**) |
| CVAE | 522.89 / 516.49 | 525.92 / 525.37 | 523.28 / 521.56 | 522.51 | 522.03 | **NO TRADE** | — |

### No trade (target <= buy) — wide context (`ctx_bars=56`, `start_idx=25736`)

![no_trade_wide](outputs/sample_plots/no_trade_wide_start25736_ctx56.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 565.93 / 564.94 | 564.94 / 561.33 | 571.01 / 571.35 | 565.90 | — | — | **BENCHMARK** → sell 571.35 (**+0.96%**) |
| PatchTST | 575.29 / 567.58 | 576.14 / 586.94 | 555.42 / 566.10 | 565.90 | 586.94 | **ENTER** | **EXPIRED** → sell 571.35 (**+0.96%**) |
| CVAE | 568.85 / 568.82 | 560.92 / 567.32 | 561.88 / 565.01 | 565.90 | 565.85 | **NO TRADE** | — |

### Lose — expiry (loss) — narrow context (`ctx_bars=28`, `start_idx=25147`)

![lose_expiry_narrow](outputs/sample_plots/lose_expiry_narrow_start25147_ctx28.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 521.67 / 520.92 | 520.92 / 521.29 | 521.29 / 520.87 | 521.67 | — | — | **BENCHMARK** → sell 520.87 (**−0.15%**) |
| PatchTST | 531.55 / 540.15 | 528.52 / 535.30 | 511.84 / 502.32 | 521.67 | 540.15 | **ENTER** | **EXPIRED** → sell 520.87 (**−0.15%**) |
| CVAE | 524.85 / 522.38 | 522.08 / 520.62 | 524.00 / 521.49 | 521.67 | 522.14 | **ENTER** | **EXPIRED** → sell 520.87 (**−0.15%**) |

### Lose — expiry (loss) — wide context (`ctx_bars=70`, `start_idx=25264`)

![lose_expiry_wide](outputs/sample_plots/lose_expiry_wide_start25264_ctx70.png)

| | Candle 1 (open / close) | Candle 2 (open / close) | Candle 3 (open / close) | Buy price | Take-profit | Trade? | vs. real price |
|---|---|---|---|---|---|---|---|
| Ground truth | 542.51 / 542.40 | 540.88 / 540.87 | 540.88 / 541.43 | 542.51 | — | — | **BENCHMARK** → sell 541.43 (**−0.20%**) |
| PatchTST | 552.91 / 542.50 | 548.79 / 546.05 | 532.28 / 542.49 | 542.51 | 546.05 | **ENTER** | **EXPIRED** → sell 541.43 (**−0.20%**) |
| CVAE | 547.85 / 544.29 | 546.42 / 549.11 | 545.45 / 540.94 | 542.51 | 545.12 | **ENTER** | **EXPIRED** → sell 541.43 (**−0.20%**) |
...
PatchTST entered all 8 this batch (its coherent-up gate cleared every time) but didn't hit its own target
on a single one of them — all 8 expired. CVAE entered 6 of 8 (skipping the 2 no-trade windows, where its
own target never cleared the buy price): 2 hit take-profit, 4 expired. This lines up with the full
backtest below, where PatchTST's take-profit resolves before expiry **under 1% of the time** at every
threshold, against CVAE's **24–39%**.
...
|---|---|---|---|---|
| Win — take-profit hit | narrow | 1.63 | 32.25 | 11.13 |
| Win — take-profit hit | wide | 2.27 | 31.17 | 9.87 |
| Win — expiry (gain) | narrow | 0.93 | 33.66 | 14.11 |
| Win — expiry (gain) | wide | 2.25 | 25.65 | 8.76 |
| No trade (target <= buy) | narrow | 2.71 | 27.70 | 9.43 |
| No trade (target <= buy) | wide | 10.02 | 31.51 | 7.93 |
| Lose — expiry (loss) | narrow | 0.80 | 37.83 | 4.23 |
| Lose — expiry (loss) | wide | 1.64 | 20.63 | 8.18 |
<!-- AUTO:spread-summary:end -->

**In plain terms**: PatchTST's generated candles are still consistently more spread out than reality
(21–38 points wide against reality's 0.8–10), and its take-profit target is pinned to the single highest
point on that already-too-wide path — a target that far out usually never gets touched by the real
3-candle range, which is exactly why it expired all 8 times above. CVAE is narrower (4–14 points) and
closer to the real range, which is part of why it resolves before expiry more often, relative to how
rarely it even enters (2 of 8 windows here didn't clear the eligibility bar at all).
...
exact same sell price (520.87) and return (−0.15%) as the Ground truth row's BENCHMARK. This is exactly
