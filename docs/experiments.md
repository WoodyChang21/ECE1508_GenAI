...
Both below build from **Run 2's exact config**, changing one thing at a time, to isolate Run 2's residual lag-echo (`corr(loc_raw, y[t-1]) = 0.16`). Since `channel_attention=False` makes `return_1h` depend only on its own patch history, the echo can only come from how that history is pooled/normalized — not from the other channels (proven inert under this config).

**Not rerun with the `patch_stride` fix** — both commits below already contain real matching output from their original run (verified 2026-08-05), and given how consistently this project's noise pattern has held across 7 independent GPU reruns, a further rerun was judged unlikely to change the conclusions enough to justify the GPU time. Treat these two as slightly less rigorously verified than Runs 1–4 above.

| # | Commit | Change from Run 2 | RMSE | MAE | Dir Acc | Cov 80% | Cov 90% | Sharpe | Max DD | corr(loc_raw, y[t-1]) |
|---|--------|--------------------|------|-----|---------|---------|---------|--------|--------|------------------------|
| 2a | [`04af727`](https://github.com/WoodyChang21/ECE1508_GenAI/commit/04af727) | Pooling: mean-pool all patches → last-patch only | 0.004151 | 0.002343 | 0.5342 | 0.8469 | 0.9255 | 0.8638 | -0.1591 | 0.0076 |
...
- **Run 2a — mixed, not a clean win.** Dir Acc/Sharpe repeat Run 3's bias signature (85% predict-up vs. 53.4% base rate, flat confidence-quartile accuracy) — treat as artifact. RMSE/MAE did improve genuinely (bias-agnostic metrics). Calibration regressed to overcovering (0.85/0.93 vs. 0.80/0.90 targets). `corr(loc_raw, y[t-1])` dropping to 0.008 is ambiguous — could be genuine fix or just a near-collapsed prediction correlating weakly with everything.
