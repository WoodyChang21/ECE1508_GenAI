...
### Architectural note: pooling mode itself may be the wrong axis

The original PatchTST paper's supervised-forecasting head **flattens every patch representation** (`vec(Z) ∈ R^{D×N}`) into the linear head, rather than pooling. HuggingFace's `PatchTSTConfig.pooling_type` defaults to `"mean"`, which this project's code never overrode — so Run 2's mean-pooling and Run 2a's `last_patch` pooling are *both* deviations from the paper, not a test of "paper vs. alternative." The paper's actual design (`pooling_type=None`, verified against `transformers` source: `modeling_patchtst.py`'s `PatchTSTPredictionHead`) lets the linear head learn arbitrary weightings across all patches — strictly more expressive than either pooling scheme tried so far, and a more principled next test of the recency-dilution hypothesis than `last_patch` was.
